...
def get_github_file(repo_name, file_path, github_token):
    ...
    # Get file to the specified repo path
    ...
    if response.status_code == 200:
        ...
        logging.info(f"{file_path} does not exist in {repo_name}. So it will be created.")
        return "", None
    else:
...
def update_github_file(repo_name, file_path, content, github_token, commit_message="Update flag file", sha=None):
    ...
    # Upload the flag to the specified repo path
    url = f"{GITHUB_API_URL}/repos/{repo_name}/contents/{file_path}"
    headers = {
        "Authorization": f"Bearer {github_token}",
        "Accept": "application/vnd.github.v3+json"
    ...
    if response.status_code in [200, 201]:
        ...
        logging.error(f"Failed to update {file_path} on GitHub repo. Status code: {response.status_code}. Response: {response.json()}")
        return False

def load_processed_ids(repo_name, github_token, file_path=FLAG_FILE_PATH):
    
    # Get list of IDs in flag.txt
    content, sha = get_github_file(repo_name, file_path, github_token)
    if content is None:
        logging.error("Unable to load processed IDs from GitHub repo.")
        sys.exit(1)
...
def save_processed_ids(repo_name, github_token, new_ids, current_content, sha, file_path=FLAG_FILE_PATH):
    ...
    # Save the IDs of retrieved images in flag.txt
    ...
    if success:
        logging.info(f"Added {len(new_ids)} new IDs to {file_path} in {repo_name}.")
    else:
        logging.error(f"Failed to add new IDs to {file_path} in {repo_name}.")

def upload_to_github(file_path, repo_name, github_token, target_folder, commit_message="Upload video file"):
    
    # Upload the retrieved video file/s to the specified repo path
    ...
    if get_response.status_code == 200:
        ...
        logging.error(f"Failed to check {file_path.name} on GitHub. Status code: {get_response.status_code}. Response: {get_response.json()}")
        return
...
def retrieve_data(supabase: Client, table_name: str = 'videos_data', data_dir: str = 'data/train_gen_vids',
    ...
    """
    Retrieves new data from Supabase, downloads videos, organizes them, uploads to GitHub repo,
    and write processed IDs to flag.txt.
    ...
    for record in tqdm(new_records, desc="Retrieving Data"):
        ...
        # Use emotion class as folder name
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if video_full_path.exists() and video_full_path.stat().st_size > 0:
                    logging.info(f"File saved successfully: {video_full_path}, Size: {video_full_path.stat().st_size} bytes")

                    upload_to_github(video_full_path, repo_name, github_token, f"{target_folder}/{emotion_normalized}")
                else:
                    logging.error(f"File was not saved or is empty: {video_full_path}")
